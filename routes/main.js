    ...
    if (nickname) {
        ...
        client.query(q, function (err, row) {
            ...
            if (row[0].is_coupled == null) {
                ...
                `diary`.`time`\
                FROM\
                    `heroku_7e0ddf49a41647e`.`diary`,\
                    `heroku_7e0ddf49a41647e`.`user`\
                WHERE\
                    (`diary`.`user_pid` = `user`.`user_pid`\
                        AND (`user`.`user_pid` =" + user_pid + "\
                        OR `user`.`user_pid` = (SELECT `match` FROM (SELECT `match` FROM `heroku_7e0ddf49a41647e`.`user` WHERE `user_pid` =" + user_pid + ") tmp)))";
                client.query(p, function (err, row) {
                    ...
                    let r = "SELECT `nickname` FROM heroku_7e0ddf49a41647e.user WHERE `match` = '"+sess.user_pid+"';";
                    client.query(r, function (err, row2) {
                        console.log("아앙아ㅏ악" + row2[0]);
...
router.post('/create', upload.single('img_url'), function (req, res, err) {
    var FILE_SIZE = 10 * 1024 * 1024; //10MB 이미지 크기 제한
    ...
    if(req.file) {
        if(req.file.size > FILE_SIZE) {
            ...
            let q = "INSERT INTO `heroku_7e0ddf49a41647e`.`diary` (`date`, `text`, `img_url`, `user_pid`, `is_deleted`)VALUES('";
            ...
            client.query(q, function (err, row) {
                if (err) throw err;
                let p = "SELECT `token` FROM `heroku_7e0ddf49a41647e`.`user` WHERE `user_pid` = " + couplePid;
                client.query(p, function (err, row) {
                    if (err) throw err;
                    console.log("대체 뭐야????????????? " + row[0]);
                    if (row[0] == undefined || row[0].token == null) {
                        ...
                        const payload = {
                            ...
                            notification: {
                                ...
                                body: msg,
                                sound: 'default',
                                click_action: 'https://parallel-diary.herokuapp.com/main/list',
                        ...
                        fcm.send(payload, function(err, response) {
                            if(err) {
                                console.error("FCM 실패 : ", err);
                                return;
                            }

                            console.log("FCM 성공 : ", response)
                        });
...
router.post('/destroy', function (res, req) {
    ...
    //이미지 삭제
    let r = "SELECT `img_url` FROM `heroku_7e0ddf49a41647e`.`diary` where `diary_pid`=" + res.body.destroy_id;
...
router.post('/update',upload.single('img_url'), function (res, req) {
    ...
    let file_q;
    let q;
    let r = "SELECT `img_url` FROM `heroku_7e0ddf49a41647e`.`diary` where `diary_pid`=" + res.body.update_id;
    if(res.file != null) {
        client.query(r, function (err, row) {
            //지난 이미지 삭제
            if(row[0].img_url != ''){
...
router.post('/updateToken', function (req, res, next) {
    ...
    FROM (\
    SELECT `user_pid`\
    FROM `heroku_7e0ddf49a41647e`.`user`\
    ...
    client.query(q, function (err, row) {
        ...
        let p = "UPDATE `heroku_7e0ddf49a41647e`.`user` SET `token` = \'" + token + "\' WHERE `user_pid` =" + sess.user_pid;
        client.query(p,function(err,row){
            if(err) {
...
router.post('/coupleMsg', function (req, res, next) {
    ...
    if(Object.keys(req.body)[0] == sess.user_pid){
        ...
        if(!Number(couplePid)) { 
            ...
            let q = "SELECT `nickname`, `is_coupled`, `token` FROM `heroku_7e0ddf49a41647e`.`user` WHERE `user_pid` =" + couplePid;
            client.query(q, function (err, row) {
                if (err) throw err;
                if (!row[0]) {
                    // 2 : 해당 유저가 존재하지 않는 에러
                    ...
                    if(row[0].is_coupled != null) {
                        ...
                        "UPDATE `heroku_7e0ddf49a41647e`.`user`\
                            ...
                            if (coupleUser == null) {
                                ...
                                const payload = {
                                    notification: {
                                        ...
                                        body: msg,
                                        sound: 'default',
                                        click_action: 'https://parallel-diary.herokuapp.com/users/login',
                                        icon: '\/icons/android-icon-192x192.png'
                                    }
                                };
                                admin.messaging().sendToDevice(coupleUser, payload).then(response => {
            ...
            if (coupleUser == null) {
                ...
                const payload = {
                    notification: {
                        ...
                        body: msg,
                        sound: 'default',
                        click_action: 'https://parallel-diary.herokuapp.com',
                        icon: '\/icons/android-icon-192x192.png'
                    }
                };
                admin.messaging().sendToDevice(coupleUser, payload).then(response => {
